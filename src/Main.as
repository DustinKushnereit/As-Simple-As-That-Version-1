package
{
	...
	{		
		...
		public var gameTimeText:TextField = new TextField;
		...
		//Sounds
		[Embed(source="../Media/Spend Some Time.mp3")]
		public var music:Class;
		
		[Embed(source="../Media/Drum 8.mp3")]
		public var boop:Class;
		
		[Embed(source = "../Media/explode.mp3")]
		public var explode:Class;
		
		[Embed(source="../Media/Drum 2.mp3")]
		public var pop:Class;
		
		public var musicSound:Sound = new music() as Sound;
		public var interactionSounds:Sound;
		
		public var volume:Number = .2;
		public var channelMusic:SoundChannel = new SoundChannel();
		public var channelSound:SoundChannel = new SoundChannel();
		public var trans:SoundTransform = new SoundTransform(volume, 0);
		...
		public var decreaseSpeed:Boolean = false;
		public var decreaseSpeedFast:Boolean = false;
		public var increaseSpeed:Boolean = false;
		public var teleportNight:Boolean = false;
		public var musicPlaying:Boolean = false;
		public var playBoop:Boolean = false;
		public var playPop:Boolean = false;
		public var playExplode:Boolean = false;
		...
		public var gameTimeDisplay:int = 0;
		...
		public function startGame(e:MouseEvent):void
		{
			...
			dimensions = 12;
			ship.graphics.lineStyle(5, 0x000000);
			ship.graphics.beginFill(0x33FF00);
		...
		{			
			if (musicPlaying)
			{
				...
				channelMusic = musicSound.play(0, 1, trans);
				musicPlaying = false;
			}
			...
			if ( hitEnemy || score <= 0 || time <= 0)
				endGame();
				
			//resizeMe(healthBarFill, lives * 15, lives * 15, true);
			
			middleObject.alpha = lives / 10;
		...
		public function checkMusic():void 
		{
			if (playBoop)
			{
				interactionSounds = new boop() as Sound;
				volume = 1;
				trans = new SoundTransform(volume, 0); 
				channelSound = interactionSounds.play(0, 1, trans);
				playBoop = false;
			}
			
			if (playExplode)
			{
				interactionSounds = new explode() as Sound;
				volume = 1;
				trans = new SoundTransform(volume, 0); 
				channelSound = interactionSounds.play(0, 1, trans);
				playExplode = false;
			...
			if (playPop)
			{
				interactionSounds = new pop() as Sound;
				volume = 1;
				trans = new SoundTransform(volume, 0); 
				channelSound = interactionSounds.play(0, 1, trans);
				playPop = false;
		...
		{			
			...
			theShield.graphics.lineStyle(3, 0x00FF00);
		...
		{	
			...
			powerUpText.text = "Shield in: " + String(countDownTimerDisplay);
			
			
			gameTimeText.defaultTextFormat = myFormat;
			gameTimeText.textColor = 0xFF0000;
			gameTimeText.x = 340;
			gameTimeText.y = 15;
			gameTimeText.width = 200;
			gameTimeText.text = "Game Time: " + String(gameTimeDisplay);
			
			scoreText.defaultTextFormat = myFormat;
			scoreText.textColor = 0x00FFFF;
			scoreText.x = 530;
			scoreText.y = 15;
			scoreText.width = 260;
			scoreText.text = "Wake Crstals Needed: " + String(score);
		}
		...
		public function createDeed():void
		{
			if ( makeDeed )
			{
				...
				deed.graphics.lineStyle(2, 0x00FFFF);
				deed.graphics.beginFill(0x00FFFF, 2);
		...
		public function createTemptation():void
		{
			if ( makeTemptation )
			{
				...
				temptation.graphics.lineStyle(2, 0xFF00FF);
				temptation.graphics.beginFill(0xFF00FF, 2);
		...
		public function makeBotRightCircle():void
		{
			...
			displayObject.graphics.drawRect(0, 0, 200, 180);
			displayObject.x = 480;
			displayObject.y = 330;
